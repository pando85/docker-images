#!/usr/bin/env bash
# Forked from:https://github.com/moby/moby/blob/master/contrib/mkimage-arch.sh
...
ROOTFS=$(mktemp -d ${TMPDIR:-/var/tmp}/rootfs-manjaro-XXXXXXXXXX)
chmod 755 $ROOTFS
...
case "$arch" in
	*)
		...
		PACMAN_MIRRORLIST='Server = http://manjaro.barata.pt/stable/$repo/$arch'
		PACMAN_EXTRA_PKGS=''
		EXPECT_TIMEOUT=60
